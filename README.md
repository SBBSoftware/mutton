...
####Add the following to your gemfile
 Note, this is pre-release, when pushed to Ruby 
gems you can remove the github address.


```
gem 'mutton', git: 'https://github.com/SBBSoftware/test-mutton.git'
```
...
##Dependencies
Sprockets. Yes, you need sprockets. If you've moved to Gulp or another pipeline solution you're going
to be unhappy.
...
##So what sucks?
All right everyone, step forward if you use form helpers. Not so fast Handlebars! Yes, in order 
to move into a world without cluttered views, **you have to give up Rails form helpers**. This sucks, sorry 
there's no more eloquent way to put it. I extracted this from a project which was almost exclusively using 
...
**The template path**. This caused more problems than the rest of the gem combined. The issue is that Rails provides 
a number of hooks for an Engines or Railties to configure settings, all but one happen after the Controllers 
have been intialized with a LookupContext. That one which is available happens before initialization files 
...
if you're doing this you will have to restart your development server. I recommend using the default setting.
...
* handlebar_assets author
* SHT authors
* ExecJS author

Finally, huge thanks to YK and the Handlebars team for the work of art that is Handlebars.
